...
for i in range(detected_objects.shape[2]):
    ...
    if confidence > min_confidence:
        ...
        cv.putText(image, prediction_text, (upper_left_x, upper_left_y - 15 if upper_left_y > 30 else upper_left_y + 15),
                   cv.FONT_HERSHEY_SIMPLEX, 0.6, colors[class_index], 2)

scale_factor = 2
enlarged_image = cv.resize(image, (image.shape[1] * scale_factor, image.shape[0] * scale_factor))

cv.imshow("Detected Objects", enlarged_image)
cv.waitKey(0)
